...
def main():
    ...
    with open(mgf_file1, 'r') as f1, open('multi_proteomer_merged_predicted_spectra.mgf', 'w') as output_file:
        # Loop over each line in file2 and write it to the output file with updated scan numbers
        for line in f1:
            if "SCAN=" in line:
                scan_number = int(line.split("SCAN=")[1])
                if scan_number >= 1:
                    new_scan_number = str(scan_number) + "-" + str(add_scan_number)
