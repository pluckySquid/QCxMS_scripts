...
# python ~/QCXMS/scripts/MSn/analysis_green.py mols_first_25_atoms.txt ~/QCXMS/workDir/neccessary_files/NIH-Nature_round2_positive/NIH_Nature_round2_positive_delete_precursor_17 merged_predicted_spectra.mgf ~/QCXMS/workDir/neccessary_files/ALL_GNPS_cleaned.csv 1 3 > debug.txtimport json
import json
import pandas as pd
...
# Yasin El Abiead
#   11:40 AM
# for expermiental: > 50 mz and 3 times min_intensity
# for simulated: >50 mz
...
def get_mgf_dict(input_mgf_file, delete_precursor_switch):
    ...
    for spectrum in mgf.read(input_mgf_file):
        ...
        intensity = [x for x in spectrum.get('intensity array')]
        ...
        if not delete_precursor_switch:
            ...
            for i in range(0, len(mz)):
                if mz[i] < float(precursor) - 17 and mz[i] > 50 and intensity[i] > 0.1:
    ...
    with open(json_smile_peak_file, "r") as json_smile_peak_file:
        for line in json_smile_peak_file:
            if str(line) != "smiles: peaks_json, atom\n":
                ...
                # denoised_intensity = [x for x in normlised_intensity if x > 0.02]
                ...
                smile_peak_dic[smile] = SpectrumTuple("0", mz_out,norm_intensity(intensity_out))
...
def get_n_largest_peaks(
    ...
    for mol in smile_peak_dict:
        ...
        sorted_intensity = [x for x in sorted_intensity]
...
# our goal is to see if a peak is explained by one or more proteomers
    ...
    for experiment_smile in describing_dict:    
        # check if there is only one proteomer, if so, this smile cannot count
