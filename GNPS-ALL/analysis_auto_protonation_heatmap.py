# python ~/QCXMS/scripts/FDA_PART2/analysis_auto_protonation_heatmap.py mols_first_10_mols.txt ~/QCXMS/workDir/neccessary_files/FDA-Part2/delete_precursor_20 merged_predicted_spectra.mgf ~/QCXMS/workDir/neccessary_files/ALL_GNPS_cleaned.csv 4 3
# python ~/QCXMS/scripts/GNPS-ALL/analysis_auto_protonation_heatmap.py mols.txt cleaned_delete_precursor_file titled_predicted_spectra_changed.mgf ~/QCXMS/workDir/neccessary_files/ALL_GNPS_cleaned.csv 4 3
import json
import pandas as pd
            ...
            for i in range(0, len(mz)):
                if mz[i] < float(precursor) - 17:
                    if noise_reduction_flag and intensity[i] > 3 * min_intensity:
                        mz_out.append(mz[i])
                        intensity_out.append(math.sqrt(intensity[i]))
                    else:
                        mz_out.append(mz[i])
                        intensity_out.append(math.sqrt(intensity[i]))
...
def plot_discription_function(describing_dict, predicted_mgf_dict, smile_peak_dict, experiment_index, smile_proteomer_dict):
    result_dict = {}
    each_smile_dict = {}
    ...
    for experiment_smile in describing_dict:
        ...
        for proteomer in describing_dict[experiment_smile]:
            ...
            for i in range(0, len(smile_peak_dict[experiment_smile+"_" + str(experiment_index)].mz)):
            ...
            if experiment_smile not in each_smile_dict.keys():
                ...
                experimental_smile_intensity_dict[experiment_smile] = (smile_peak_dict[experiment_smile+"_" + str(experiment_index)].mz, smile_peak_dict[experiment_smile+"_" + str(experiment_index)].intensity)
    ...
    for smile in predicted_smile_intensity_dict:
        ...
        for i, (mz_values, intensity_values) in enumerate(smile_list):
            proteomer_name = smile_proteomer_dict[smile][counter]

            ax = axs[i]  # Select the appropriate subplot
            if len(intensity_values) == 0:
                print("empty intensity_values")
                break
...
#            bar_width = np.min(np.diff(mz_values)) * 800
            ...
            ax.set_title(f'm/z Intensity Bar Graph for ' + proteomer_name)# + f'_{i+1}')
...
#        bar_width = np.min(np.diff(smile_list[0])) * 10
        ...
        # Save the figure
        try:
            plt.savefig(str(smile)+"-exp_"+str(experiment_index) +'_intensity.png')
        except:
...
def get_smile_proteomer_dict(input_mgf_file, scan_smile_dict):
    smile_proteomer_dict = {}
    count = 0
    ...
    for spectrum in mgf.read(input_mgf_file):
        ...
        params = spectrum.get('params')
        scan = params['scan']
        proteomer = params['title']
        print("proteomer:", proteomer)

        print("scan_smile_dict.keys():", scan_smile_dict.keys())

        smile = scan_smile_dict[scan.split('-')[0]] 

        print("smile:", smile)
        if smile not in smile_proteomer_dict.keys():
            smile_proteomer_dict[smile] = [proteomer]
        else:
            smile_proteomer_dict[smile].append(proteomer)
        count = count + 1

    return smile_proteomer_dict
...
def main():
    ...
    smile_proteomer_dict = get_smile_proteomer_dict(input_mgf_file, scan_smile_dict)
    ...
    for experiment_index in range(1, 100):
        # try:
            ...
            for i in range(1, len(scan_smile_dict)+1) :
                ...
                if smile + "_" + str(experiment_index )not in smile_peak_dict.keys():
                    print("this smile is out of range:", smile + "_" + str(experiment_index))
                    continue
                ...
                if str(i) in mgf_keys:        
                    ...
                    while(True):
                        ...
                        #print("spec1: ", spec1)
                        spec2=smile_peak_dict[smile+"_" + str(experiment_index)]
                        #print(spec2)
            ...
            for smile in describing_true_peak_dict.keys():       
                ...
                for i in describing_true_peak_dict[smile]:
                    describing_mz_dict[smile].append(smile_peak_dict[smile+"_" + str(experiment_index)].mz[i])
                for i in describing_true_peak_dict[smile]:
                    describing_intensity_dict[smile].append(smile_peak_dict[smile+"_" + str(experiment_index)].intensity[i])
                ...
                for index, i in enumerate(smile_peak_dict[smile+"_"+str(experiment_index)].intensity):
                    if index not in overlap_index_set:
                        non_overlap_intensity_list.append(i)
                        non_overlap_mz_list.append(smile_peak_dict[smile+"_" + str(experiment_index)].mz[index])
                ...
                plt.savefig('hist'+  str(experiment_index) + "-" +str(counter) + '.png')
                counter += 1
                plt.clf()
            ...
            for i in describing_true_peak_dict.keys():
                print("len describing_true_peak_dict i: ", i, len(describing_true_peak_dict[i]), len(smile_peak_dict[i+"_" + str(experiment_index)].mz))
                #print("len smile_peak_dict[smile+ + str(counter)] i: ", i, len(smile_peak_dict[i+"_1"].mz))

            plot_discription_function(describing_dict, predicted_mgf_dict, smile_peak_dict, experiment_index, smile_proteomer_dict)
