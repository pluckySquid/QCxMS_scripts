...
def main():
    ...
    for item in data:
        ...
        if 'Smiles' in str(item):
            ...
            if smile in smile_dict.keys() and "M+H" in Adduct:# and "Orbitrap" in instrument:     
                #print("smile ", smile_dict[smile])
                smile_dict[smile] = smile_dict[smile] + 1
            elif "M+H" in Adduct:# and "Orbitrap" in instrument:
                smile_dict[smile] = 1
            else:
                continue
    ...
    for key, value in smile_dict.items():
        ...
        if "N/A" not in str(key) and str(key) != ' ':
            ...
            try: 
                ...
                if atom_number >= 36 and atom_number <= 37:
                    mol_atom_dict[str(key)] = atom_number
                    mol_time =  0.733 * atom_number * atom_number - 10.66 * atom_number
